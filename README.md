...
### 1. 배경 및 필요성 
MZ 세대 소비 성향이 변화하며 새로운 리셀 시장이 등장했고 리셀 상품 재판매는 "리셀테크"라고 불리며 새로운 재테크로 떠오르는 중
다른 재테크 방법들보다는 진입이 쉬우나 급격히 성장하게 된 시장으로 투자와 관련한 정보 부족
...
- 시계열 데이터(기간 내 체결 거래 내역) 수집에 어려움이 있어 상품별 수익성 예측 등과 같은 분석을 진행하지 못한 점
- 눈에 확연히 보일 만한 군집별 명확한 차이가 없는 점

개선방안
- 거래 내역 데이터 수집 후 상품별로 추가적인 수익성 예측 진행
- 
